...
# 或
pnpm add tracker-sdk-taro-weapp
# 或
yarn add tracker-sdk-taro-weapp
```
...
{
  "dependencies": {
    "tracker-sdk-taro-weapp": "file:path/to/tracker-sdk/release/tracker-sdk-taro-weapp"
  }
}
```
...
```typescript
import { createTaroWeappTracker } from 'tracker-sdk-taro-weapp'
...
```typescript
import * as TrackerSDK from '@/assets/js/taro-weapp.mjs'

const tracker = (TrackerSDK as any).createTaroWeappTracker({
  appId: 'your_app_id',
  endpoint: 'https://your-tracking-endpoint.com/api/track',
  debug: process.env.NODE_ENV === 'development',
  mock: process.env.NODE_ENV === 'development'
...
// 或使用本地文件
// import * as TrackerSDK from '@/assets/js/taro-weapp.mjs'
// const { createTaroWeappTracker } = TrackerSDK as any
...
#### `setTenantId(tenantId | TenantInfo)`

设置租户身份，用于按租户维度关联用户行为数据。除 `tenantId` 外，还支持传入对象注册额外扩展字段（如 `sessionId`、`robotId` 等），扩展字段会持久化到本地，并在每次上报时作为顶层字段携带。
...
// 创建 tracker 实例
export const tracker = createTaroWeappTracker(trackerConfig)
// 或使用本地文件
// export const tracker = (TrackerSDK as any).createTaroWeappTracker(trackerConfig)
...
- **v0.2.1**: 租户信息支持扩展字段
  - `setTenantId` 支持传入 `TenantInfo` 对象
  - 可注册 `sessionId`、`robotId` 等自定义字段
  - 扩展字段持久化并在上报数据中作为顶层字段携带
